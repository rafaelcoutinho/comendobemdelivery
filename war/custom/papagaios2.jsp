	...
	if (str != null) {
		...
		if(delCostMatrix!=null){
			...
			for(i=0;i<matrix.length;i++){
				out.println(matrix[i]+"<br>");
				StringTokenizer stokenPlate = new StringTokenizer(matrix[i],"|");
				
				
				JsonObject dataJson = new JsonObject();
				String name=stokenPlate.nextToken();
				dataJson.add("name",new JsonPrimitive(name));
				...
				try{
					String actualname= nm.get(KeyFactory.stringToKey(id)).getName();
					if(actualname.equals(name)){
						out.println(actualname);		
					}else{
						out.println("<span style='color:red'>"+actualname+"</span>");
					}
				out.println("<br>--<br>");
				}catch(Exception e){
					out.println(id+": NOT FOUND<br>--<br>");	
				}
				id=id.replace('\n',' ');
				id=id.trim();
				taxesjson.add(id,dataJson);
				out.println(id+":"+dataJson.toString()+"<br>_______<br>");
			}
