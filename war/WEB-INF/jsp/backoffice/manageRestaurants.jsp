...
<ul>
		<c:forEach items="${entity}" var="rbean" varStatus="status">
			<li>${rbean.restaurant.name}- <a href="#"
				onclick="loginAs('${rbean.restaurantKey}','${fn:replace(rbean.restaurant.name,'\'', '\\\'')}');returnfalse;">gerenciar</a></li>
		</c:forEach>
	</ul>
...
function loginAs(id,name){
	...
	console.info(a);
	if(a==true){
		var form = dojo.create("form",{action:"/backoffice/gerenciarRestaurante.do",method:"post"},dojo.byId('hiddenSection'));
