...
<cb:body closedMenu="true">
	...
	<form action="/backoffice/fechamento.do" method="GET" id="factureForm"
		style="text-align: center;" >
		<input type="checkbox" name="includeERP" value="true" checked="checked"><label for="includeERP">Incluir pedidos ERP</label> <br/>
	...
	<c:set var="totalPedidosOnline" value="0"/>
	<c:set var="numTotalPedidosOnline" value="0"/>
	<c:forEach items="${entity.ordersByRestaurant}" var="restBillBean" varStatus="status">
		...
		<c:set var="subTotalPedidos" value="0"/>
		<c:set var="subTotalPedidosOnline" value="0"/>
		<c:set var="subNumTotalPedidosOnline" value="0"/>
		...
		<div class="tableCol">R$ <fmt:formatNumber pattern="##.##" minFractionDigits="2">
		<c:out value="${(paymentTypeTotal.value[0]/100)}" default="0"/></fmt:formatNumber> <c:if test="${paymentTypeTotal.key=='PAYPAL'}">(R$ <fmt:formatNumber pattern="##.##" minFractionDigits="2"><c:out value="${paymentTypeTotal.value[3]/100}" default="0"/></fmt:formatNumber>)</c:if></div>
		<div class="tableCol" style="display:inline">${paymentTypeTotal.value[2]} (${paymentTypeTotal.value[5]})</div>
		<c:set var="subNumTotalPedidosOnline" value="${subNumTotalPedidosOnline + paymentTypeTotal.value[5]}"/>
		<c:set var="totalPedidos" value="${totalPedidos+paymentTypeTotal.value[2]}"/>
		<c:set var="subTotalPedidos" value="${subTotalPedidos + paymentTypeTotal.value[2]}"/>
		<c:set var="subTotalPedidosOnline" value="${subTotalPedidosOnline + paymentTypeTotal.value[4]}"/>
		</div>
		</c:forEach>
		<div style="width: 600px;">	
		<div class="tableCol"></div>
		<div class="tableCol" style="font-style: italic;text-align: right;">SubTotal:</div>	
		<div class="tableCol" style="font-style: italic;">R$ <fmt:formatNumber pattern="##.##" minFractionDigits="2"><c:out value="${restBillBean.totals/100}" default="0"/></fmt:formatNumber> (<fmt:formatNumber pattern="##.##" minFractionDigits="2"><c:out value="${subTotalPedidosOnline/100}" default="0"/></fmt:formatNumber>)</div>
		
		<div class="tableCol" style="display:inline;width: 200px"><i>${subTotalPedidos}</i> (${subNumTotalPedidosOnline })</div>
		</div> 
		</div>	
		<c:set var="numTotalPedidosOnline" value="${numTotalPedidosOnline + subNumTotalPedidosOnline}"/>
	...
	<div class="tbentry" style="background-color:rgb(240,240,240);font-size: large;font-weight: bold; "> 
		...
		<div class="tableCol" style="display:inline"><i>${totalPedidos}</i> (${numTotalPedidosOnline })</div>
		</div>
