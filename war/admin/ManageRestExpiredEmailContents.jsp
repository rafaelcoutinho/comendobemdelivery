...
<%
	String msg = "";
	String subjectConfKey = "expired.order.rest.email.subject";
	String msgConfKey = "expired.order.rest.email.msg";
	...
	String emailMsg = br.copacabana.usecase.MealOrderExpirer.REST_MSG;
	
	if(((Configuration)cman.find(msgConfKey, Configuration.class))!=null){	
		emailMsg = ((Configuration)cman.find(msgConfKey, Configuration.class)).getValue();
	}
	
	String emailSubject = br.copacabana.usecase.MealOrderExpirer.REST_SUBJECT;
	if(((Configuration)cman.find(subjectConfKey, Configuration.class))!=null){	
		emailSubject = ((Configuration)cman.find(subjectConfKey, Configuration.class)).getValue();
	}
...
<a href="CreateWelcomeEmailContents.jsp">Email boas vindas</a> - <a href="CreateExpiredEmailContents.jsp">Email pedido vencido Cliente</a> -  <a href="ManageRestExpiredEmailContents.jsp">Email pedido vencido Restaurante</a> - <a href="CreateCancelledEmailContents.jsp">Email de pedido cancelados</a> <br> 
<h2>Edicao do conteudo do email para Restaurante quando pedidos para ele vencem.</h2><br>
...
Assunto :<input type="hidden" name="key" value="<%=subjectConfKey%>" readonly="readonly" dojoType="dijit.form.TextBox"/> = <input type="text" name="value" value="<%=emailSubject%>" dojoType="dijit.form.TextBox"/> 
<input type="button" value="Salvar" onclick="submitForm('subjectForm')">
<div id="logsubjectForm"></div>
</form>
<form
	action="ManageRestExpiredEmailContents.jsp" onsubmit="moveValues();return false;" id="msgForm">
	<input type="hidden" name="id" value="<%=msgConfKey%>" />
	Mensagem:<br />
