...
Filtrando por cidade: <%=filterCity %><br>
Excluindo null: <%=excludeNull %><br>
<form action="NewsLetterClients.jsp">
Filtrar por cidade:<br/>
...
<input  type="checkbox" name="excludeNull" <%=checked %>>Excluir sem endereços
	...
	<%
				...
				for (Iterator<Client> iter = clist.iterator(); iter.hasNext();) {
					...
					if (filterCity == true) {
						...
						if (excludeNull == false && (c.getAddresses() == null || c.getAddresses().size() == 0)) {
							...
							for (Iterator<Key> iter2 = c.getAddresses().iterator(); iter2.hasNext();) {
								...
								if (add.getNeighborhood().getCity().getIdStr().equals(cityKey)) {
									contem = true;
									break;
								}
							}
